...
resource "openstack_compute_secgroup_v2" "private_security_group" {
  name = "private_security_group"
  description = "an private security group"
  rule {
    ip_protocol = "tcp"
    from_port   = 22
    to_port     = 22
...
resource "openstack_compute_instance_v2" "private_host" {
  name            = "private_host"
  count = 1
  ...
  flavor_id       = "7d73f524-f9a1-4e80-bedf-57216aae8038"
  key_pair        = "${openstack_compute_keypair_v2.keypair.name}"
  security_groups = ["${openstack_compute_secgroup_v2.private_security_group.name}"]
